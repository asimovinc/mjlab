...
def asimov_1_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  ...
  # Only the site-velocity rewards take an asset_cfg. foot_swing_height uses
  # the foot height sensor directly, so it does not have per-site params here.
  ...
  if play:
    ...
    if "reset_base" in cfg.events:
      ...
      params["velocity_range"] = {
        k: (0.0, 0.0) for k in params.get("velocity_range", {})
      }
